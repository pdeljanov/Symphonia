...
use crate::{RefProcess, VideoTestDecoder, VideoTestOptions, get_codec_type, to_ref_s};
...
fn run_test(path: &str, opts: &VideoTestOptions, result: &mut VideoTestResult) -> Result<()> {
    ...
    let Some(video_track) = format.first_track(TrackType::Video)
    else {
        println!("No Video track found - skip validation");
        return Ok(());
    };
        ...
        let act = loop {
            match format.next_packet() {
                ...
                Ok(None) => {
                    ...
                    return decode_error(
                        "video: Symphonia reached end of file but reference decoder still have packets",
                    );
                }
                Err(Error::IoError(err)) if err.kind() == std::io::ErrorKind::UnexpectedEof => {
                    ...
                    return decode_error(
                        "video: Symphonia reached end of file wav, but reference decoder still have packets",
                    );
                }
                Err(err) => {
