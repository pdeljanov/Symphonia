...
use symphonia::core::codecs::subtitle::{SubtitleCodecParameters, well_known::*};
use symphonia::core::codecs::video::VideoCodecId;
use symphonia::core::codecs::video::{VideoCodecParameters, well_known::*};
...
mod mediainfo;
use crate::{InfoTestDecoder, InfoTestOptions, RefProcess, get_codec_type, to_ref_s};
...
fn compare_track(diff_lines: &mut Vec<Line>, act: &Track, exp: &Track) {
    ...
    if !equal_duration(act, exp) {
        diff_lines.push(Line::new_s("Duration".to_string(), get_duration(act), get_duration(exp)));
    }
}

fn equal_duration(act: &Track, exp: &Track) -> bool {
    match (act.time_base, act.num_frames, exp.time_base, exp.num_frames) {
        (Some(act_time_base), Some(act_num_frames), Some(exp_time_base), Some(exp_num_frames)) => {
            let act_time = act_time_base.calc_time(Timestamp::new(act_num_frames as i64)).unwrap();
            let exp_time = exp_time_base.calc_time(Timestamp::new(exp_num_frames as i64)).unwrap();
            act_time.as_millis().abs_diff(exp_time.as_millis()) < 2
        }
        _ => false,
    }
}
...
fn get_duration(tr: &Track) -> String {
    match (tr.time_base, tr.num_frames) {
        (Some(timebase), Some(ts)) => {
            let time = timebase.calc_time(Timestamp::new(ts as i64)).unwrap();
            // Format with 3 decimal places
            format!("{}.{:03}", time.as_millis() / 1000, time.as_millis() % 1000)
        }
        _ => "None".to_string(),
    }
}
