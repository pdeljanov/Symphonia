...
use symphonia::core::codecs::subtitle::{CODEC_ID_NULL_SUBTITLE, SubtitleCodecParameters};
use symphonia::core::codecs::video::well_known::*;
use symphonia::core::codecs::video::{CODEC_ID_NULL_VIDEO, VideoCodecParameters};
use symphonia::core::errors::{Result, decode_error};
use symphonia::core::formats::well_known::*;
use symphonia::core::formats::{
    FORMAT_ID_NULL, FormatInfo, FormatReader, SeekMode, SeekTo, SeekedTo, Track,
};
...
fn add_track(tracks: &mut Vec<Track>, tr: &MediainfoTrack) {
    ...
    // priority take source_duration, duration, then mdhd_duration
    let duration =
        tr.source_duration.as_ref().and_then(|duration| parse_duration(duration.as_str())).or_else(
            || {
                tr.duration.as_ref().and_then(|duration| parse_duration(duration.as_str())).or_else(
                    || {
                        tr.extra
                            .as_ref()
    ...
    if !skip_track {
        ...
        track.with_time_base(TimeBase::new(NonZero::new(1).unwrap(), NonZero::new(1000).unwrap()));
...
fn parse_duration(duration: &str) -> Option<u64> {
    // duration is nominated in seconds plus milliseconds if it is a fraction (2344.233)
    // or just in milliseconds (2344233) if it's integer
...
fn get_v_codec_params(tr: &MediainfoTrack, id: u32) -> Option<CodecParameters> {
    if let Some(format) = &tr.format {
        let codec = match format.as_str() {
            ...
            other => {
                println!(
                    "mediainfo: symphonia doesn't detect video codec: \"{}\", with codec_id: {:?} for track_id: {}",
                    other, tr.codec_id, id
                );
                CODEC_ID_NULL_VIDEO
            }
        };
...
fn get_a_codec_params(tr: &MediainfoTrack, tr_id: u32) -> Option<CodecParameters> {
    if let Some(format) = &tr.format {
        let codec = match format.as_str() {
            ...
            "PCM" => match (tr.format_settings.as_deref(), tr.bit_depth.as_deref()) {
                ...
                (format_settings, bit_depth) => {
                    println!(
                        ...
                        format_settings, bit_depth, tr_id
                    );
                    CODEC_ID_NULL_AUDIO
                }
            },
            ...
            other => {
                println!(
                    "mediainfo: symphonia doesn't detect audio codec: {}, with codec_id: {:?} for track_id: {}",
                    other, tr.codec_id, tr_id
                );
                CODEC_ID_NULL_AUDIO
            }
        };
...
fn get_s_codec_params(tr: &MediainfoTrack, id: u32) -> Option<CodecParameters> {
    if let Some(format) = &tr.format {
        let codec = match format.as_str() {
            ...
            other => {
                println!(
                    "mediainfo: symphonia doesn't detect subtitle codec: {}, with codec_id: {:?} for track_id: {}",
                    other, tr.codec_id, id
                );
            ...
            let (format, short_name) = match format.as_str() {
                ...
                "MPEG Audio" => {
                    ...
                    if let Some(format_profile) = &first.format_profile {
                        ...
                        println!(
                            "mediainfo: first track don't have \"Format_Profile\" for \"MPEG Audio\""
                        );
                        (FORMAT_ID_NULL, "Unknown")
                    }
                }
