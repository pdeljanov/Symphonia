...
const MAX_HUFFMAN_SAMPLE: usize = 8207;

lazy_static! {
    /// Lookup table for computing x(i) = s(i)^(4/3) where s(i) is a decoded Huffman sample. The
    /// value of s(i) is bound between 0..8207.
    static ref REQUANTIZE_POW43: Box<[f32; MAX_HUFFMAN_SAMPLE]> = {
        (0..MAX_HUFFMAN_SAMPLE).map(|i| f32::powf(i as f32, 4.0 / 3.0)).collect::<Vec<f32>>().into_boxed_slice().try_into().unwrap()
    };
}
