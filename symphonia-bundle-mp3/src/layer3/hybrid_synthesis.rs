...
/// ```
static IMDCT_WINDOWS: std::sync::LazyLock<[[f32; 36]; 4]> = std::sync::LazyLock::new(|| {
...
static IMDCT_HALF_COS_12: std::sync::LazyLock<[[f32; 6]; 6]> = std::sync::LazyLock::new(|| {
...
/// ```
static ANTIALIAS_CS_CA: std::sync::LazyLock<([f32; 8], [f32; 8])> =
...
pub(super) fn antialias(channel: &mut GranuleChannel, samples: &mut [f32; 576]) {
    ...
    // Amortize the LazyLock fetch over the entire anti-aliasing operation.
