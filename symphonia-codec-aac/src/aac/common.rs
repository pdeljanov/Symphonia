...
/// 960-sample subband info, indexed by sample rate (same structure as `AAC_SUBBAND_INFO`).
#[rustfmt::skip]
const AAC_SUBBAND_INFO_960: [GASubbandInfo; 12] = [
...
impl GASubbandInfo {
    ...
    /// Find the subband info for the given sample rate using 960-sample tables.
    pub fn find_960(srate: u32) -> GASubbandInfo {
        for sbi in AAC_SUBBAND_INFO_960.iter() {
            if srate >= sbi.min_srate {
                return *sbi;
            }
        }
