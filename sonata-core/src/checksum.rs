...
/// `Crc32` implements the CRC32 checksum algorithm with the parameters stated below.
///
...
impl Crc32 {
    /// Instantiate a Crc32 instance with an initial state.
...
impl Monitor for Crc32 {
    #[inline(always)]
    fn process_byte(&mut self, byte: u8) {
        self.state = (self.state << 8) ^ CRC32[0][usize::from((self.state >> 24) as u8 ^ byte)];
    }
    ...
    fn process_buf_bytes(&mut self, buf: &[u8]) {
        ...
        for bytes in &mut iter {
            ...
            self.state = CRC32[7][((xor >> 24) & 0xff) as usize] ^
                         CRC32[6][((xor >> 16) & 0xff) as usize] ^
                         CRC32[5][((xor >>  8) & 0xff) as usize] ^
                         CRC32[4][((xor >>  0) & 0xff) as usize] ^
                         CRC32[3][usize::from(bytes[4])] ^
                         CRC32[2][usize::from(bytes[5])] ^
                         CRC32[1][usize::from(bytes[6])] ^
                         CRC32[0][usize::from(bytes[7])];
        }
...
#[cfg(test)]
mod tests {
    use super::{Crc32, Monitor};

    #[test]
    fn verify_crc32() {
        // Test using CRC32/MPEG2 parameters.
        {
            let mut crc = Crc32::new(0xffffffff);
            crc.process_buf_bytes(&[]);
            assert_eq!(crc.crc(), 0xffffffff);
        }
        {
            let mut crc = Crc32::new(0xffffffff);
            crc.process_buf_bytes(&[0]);
            assert_eq!(crc.crc(), 0x4e08bfb4);
        }
        {
            let mut crc = Crc32::new(0xffffffff);
            crc.process_buf_bytes(b"123456789");
            assert_eq!(crc.crc(), 0x0376e6e7);
        }
        {
            let mut crc = Crc32::new(0xffffffff);
            crc.process_buf_bytes(b"abcdefghijklmnopqrstuvwxyz123456789");
            assert_eq!(crc.crc(), 0x178dc1e0);
        }
        {
            let mut crc = Crc32::new(0xffffffff);
            crc.process_byte(b'1');
            crc.process_byte(b'2');
            crc.process_byte(b'3');
            crc.process_byte(b'4');
            crc.process_byte(b'5');
            crc.process_byte(b'6');
            crc.process_byte(b'7');
            crc.process_byte(b'8');
            crc.process_byte(b'9');
            assert_eq!(crc.crc(), 0x0376e6e7);
        }
    }
}